...
public class LongestPalindromicSubstringLength { 
    static int longestPalindromicSubstringLength(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            int len1 = expandAroundCenter(s, i, i);    // Assume odd length, try to expand Palindrome centered at i
    ...
    public static void main(String[] args) { 
        ...
        for (int tests = 0; tests < t; tests++) {
            int n = sc.nextInt(); // Read but unused as we read full string next
            String s = sc.next();
