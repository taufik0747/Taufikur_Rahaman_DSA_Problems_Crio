...
public class BeesToHives{
   ...
   Collections.sort(beesPositions);
        ...
        // Find the maximum distance a bee needs to travel to get to its corresponding hive
        int maxTime = 0;
        for (int i = 0; i < n; i++) {
            maxTime = Math.max(maxTime, Math.abs(beesPositions.get(i) - hivesPosition.get(i)));
        }
