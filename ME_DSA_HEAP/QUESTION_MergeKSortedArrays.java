...
class MergeKSortedArrays{
    ...
    static ArrayList<Integer> mergeKSortedArrays(ArrayList<ArrayList<Integer>> arrays){
       ArrayList<Integer> ans = new ArrayList<>();
        // Priority queue (min-heap) to keep track of the smallest element and its origin array
        ...
        while (!minHeap.isEmpty()) {
            ...
            ans.add(e.value);
            int nextIndex = e.indexInArray + 1;
            if (nextIndex < arrays.get(e.arrayIndex).size()) {
                minHeap.add(new Element(arrays.get(e.arrayIndex).get(nextIndex), e.arrayIndex, nextIndex));
            }
        }
...
// Helper class to store the value along with its array and index position
class Element {
    int value;
    int arrayIndex;
    int indexInArray;

    Element(int value, int arrayIndex, int indexInArray) {
        this.value = value;
