...
class MaxAreaOfIsland {
    ...
    public static int maxAreaOfIsland(ArrayList<ArrayList<Integer>> grid) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                // If the cell is '1' and not visited, calculate the area of the island
    ...
    private static int dfs(ArrayList<ArrayList<Integer>> grid, boolean[][] visited, int x, int y, int n, int m) {
        ...
        while (!stack.isEmpty()) {
            ...
            for (int k = 0; k < 4; k++) {
                ...
                if (newX >= 0 && newX < n && newY >= 0 && newY < m && grid.get(newX).get(newY) == 1 && !visited[newX][newY]) {
