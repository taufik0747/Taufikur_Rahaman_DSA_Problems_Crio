...
class LongestSubstringWithoutRepeatingCharacter {
  public int lengthOfLongestSubstring(String s) {
        ...
        for (int end = 0; end < s.length(); end++) {
            ...
            // If the character is found in the map and the index is within the current window
            if (charIndexMap.containsKey(currentChar) && charIndexMap.get(currentChar) >= start) {
                start = charIndexMap.get(currentChar) + 1;  // Move the start right after the last occurrence
            }

            charIndexMap.put(currentChar, end);  // Update the latest index of the character
